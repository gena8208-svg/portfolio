...
if ($office != 'P0') {
    $stmt = $pdo->prepare("SELECT id, date, name, site_id, user_id, cash, kassa, acquiring, transfer,payment_account, shipment, comment, manager_name, office FROM kassa WHERE office = :office ORDER BY id DESC LIMIT 300");
    ...
    $stmt = $pdo->prepare("SELECT id, date, name, site_id, user_id, cash, kassa, acquiring, transfer, payment_account, shipment, comment, manager_name, office FROM kassa ORDER BY id DESC LIMIT 300");
    $stmt->execute();
    $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
