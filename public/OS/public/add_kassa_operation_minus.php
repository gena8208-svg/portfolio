...
try {
    ...
    $logMessage .= "id клиента: " . $site_id . ", Нал: " . $nal . ", Касса: " . $kassa . ", Эквайринг: " . $ekv . ", Перевод: " . $transfer . ", Расчетный_счет: " . $payment_accountminus . ", comment: " . $comment . ", office: " . $office . ", Менеджер: " . $manager_name . "\n";
    error_log($logMessage, 3, 'logs/spisanie_v_kasse.log', FILE_APPEND);
    $sumallinputs = (int)$nal + (int)$kassa + (int)$ekv + (int)$transfer + (int)$payment_accountminus;
    ...
    $logMessage .= "id Клиента: " . $site_id . ", Баланс_новый: " . $balance . "\n";
    error_log($logMessage, 3, 'logs/spisanie_v_kasse.log', FILE_APPEND);
    $pdo->commit();
