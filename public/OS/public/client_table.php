<?php
header('Content-Type: application/json; charset=utf-8');
require_once __DIR__ . '/../config/boot.php';
if (!isset($_SESSION['username'])) {
    header('Location: login.php');
    exit;
}
$pdo = pdo();
$office = $_SESSION['office'];
$site_id = $_SESSION['site_id'];
/* $name = $_SESSION['username']; */

if ($office == 'CL') {
    /* $stmt = $pdo->prepare("SELECT id, balance_limit FROM users WHERE site_id = :site_id AND name = :name"); */
    $stmt = $pdo->prepare("SELECT id, balance_limit FROM users WHERE site_id = :site_id");
    $stmt->bindParam(':site_id', $site_id);
    /*  $stmt->bindParam(':name', $name); */
    $stmt->execute();

    $client = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($client) {
        $client_id = $client['id'];
        $balance_limit = $client['balance_limit'];
    }

    // Шаг 2: Подсчет итогов по всей таблице kassa
    $stmt = $pdo->prepare("SELECT SUM(cash) AS total_cash, SUM(shipment) AS total_shipment, SUM(transfer) AS total_transfer 
FROM kassa WHERE user_id = :user_id");
    $stmt->bindParam(':user_id', $client_id);
    $stmt->execute();

    $totals = $stmt->fetch(PDO::FETCH_ASSOC);

    $totalCash = $totals['total_cash'] ?? 0; // Если нет данных, ставим 0
    $totalTransfer = $totals['total_transfer'] ?? 0;
    $totalShipment = $totals['total_shipment'] ?? 0;
    // Шаг 3: Фильтрация по client_id в таблице kassa
    $stmt = $pdo->prepare("SELECT id, date, cash, shipment, transfer, comment 
    FROM kassa 
    WHERE user_id = :user_id
    ORDER BY id DESC 
    LIMIT 500");
    $stmt->bindParam(':user_id', $client_id);
    $stmt->execute();
    $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $totalSum = $totalCash + $totalShipment + $totalTransfer;
    // Шаг 4: Подготовка ответа
    $response = [
        'data' => $results,
        'totals' => [
            'total_cash' => $totalCash,
            'total_shipment' => $totalShipment,
            'total_sum' => $totalSum,
            'total_transfer' => $totalTransfer,
            'balance_limit' => $balance_limit,
        ]
    ];
    header('Content-Type: application/json');
    echo json_encode($response);
} elseif ($office == 'CLU') {
    // Шаг 1: Получение client_id из таблицы клиентов
    $stmt = $pdo->prepare("SELECT id, balance_limit FROM users WHERE site_id = :site_id");
    $stmt->bindParam(':site_id', $site_id);
    $stmt->execute();
    $client = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($client) {
        $client_id = $client['id'];
        $balance_limit = $client['balance_limit'];
    }

    // Шаг 2: Подсчет итогов по всей таблице kassa
    $stmt = $pdo->prepare("SELECT SUM(payment_account) AS total_payment, SUM(shipment) AS total_shipment  FROM kassa WHERE user_id = :user_id");
    $stmt->bindParam(':user_id', $client_id);
    $stmt->execute();

    $totals = $stmt->fetch(PDO::FETCH_ASSOC);
    $totalShipment = $totals['total_shipment'] ?? 0;
    $total_payment = $totals['total_payment'] ?? 0;
    $totalSum = $totalShipment + $total_payment;
    // Шаг 3: Фильтрация по client_id в таблице kassa
    $stmt = $pdo->prepare("SELECT id, date, payment_account, shipment, comment 
                                    FROM kassa 
                                    WHERE user_id = :user_id
                                    ORDER BY id DESC 
                                    LIMIT 500");

    $stmt->bindParam(':user_id', $client_id);
    $stmt->execute();

    $results = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Шаг 4: Подготовка ответа
    $response = [
        'data' => $results,
        'totals' => [
            'total_shipment' => $totalShipment,
            'total_payment' =>  $total_payment,
            'total_sum' => $totalSum,
            'balance_limit' => $balance_limit,
        ]
    ];

    header('Content-Type: application/json');
    echo json_encode($response);
} else {
    // Клиент не найден - перенаправление на login.php
    header('Location: login.php');
    exit(); // Завершение скрипта после перенаправления
}
